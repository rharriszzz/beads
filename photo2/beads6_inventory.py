#!/usr/bin/env python3
"""Image-only beads6 body/color review with R069 sliver selection."""
...
SUPPORT_CLASS={'red':1,'blue-gray':2,'white':3}
PARAMETERS=dict(envelope_closing_radius=10,envelope_fill_holes_below=1500,
                minimum_red_dominance=25,minimum_blue_dominance=12,minimum_saturation=.13,minimum_neutral_value=95,
...
def color_support(rgb,envelope):
    ...
    blue=(np.minimum(rgb[:,:,1],rgb[:,:,2]).astype(int)-rgb[:,:,0].astype(int)>=PARAMETERS['minimum_blue_dominance'])&(s>=PARAMETERS['minimum_saturation'])&envelope
    channels=np.zeros(envelope.shape,np.uint8)
    channels[envelope&(v*255>=PARAMETERS['minimum_neutral_value'])]=3
    channels[red]=1;channels[blue]=2
    for index,core in ((1,red),(2,blue)):
        components,_=ndi.label(~core);sizes=np.bincount(components.ravel())
        fill=sizes<=PARAMETERS['fill_color_highlight_holes_below']
        fill[np.unique(np.r_[components[0],components[-1],components[:,0],components[:,-1]])]=False
        channels[fill[components]&envelope]=index
    return red|blue,channels>0,channels
...
def correction_sheet(rgb,old,active,path):
    boxes=[('White bodies missed by brightness peaks', (440,65,515,118)),
           ('Top white bodies', (265,60,320,123)),
           ('White strip on upper-right bend', (565,142,640,191)),
           ('Lower white bodies', (373,418,428,485))]
    ...
    for i,(title,box) in enumerate(boxes):
        ...
        for j,(label,rows) in enumerate([('Raw JPEG',[]),('R059 candidates',before),('R078 active',active)]):
...
def gallery(path,active,excluded,summary,boxes):
    encoded=base64.b64encode((ROOT/'beads6.jpg').read_bytes()).decode()
    parts=['<!doctype html><meta charset="utf-8"><title>beads6 active inventory</title>',
           ...
           '<h1>beads6.jpg: active body observations</h1>',
    ...
    parts.append('</svg><h2>Unresolved region warnings</h2><img src="warnings.png" alt="Raw, numbered and outlined warning regions"><h2>Corrections from JPEG review</h2><img src="corrections.png" alt="Raw, baseline and active observations"><h2>Local-area exclusions</h2><img src="area-review.png" alt="Small regions excluded by the unchanged area rule"><h2>Foreground and palette</h2><img src="envelope.png" alt="Cyan search envelope"><img src="palette.png" alt="Red, blue-gray and white support"><h2>Provisional active regions</h2><p>Yellow: region boundaries. Excluded regions remain unassigned; same-color borders are approximate.</p><img src="regions.png" alt="Active region boundaries">')
    for name in boxes:parts.append(f'<h2>{name}</h2><img src="{name}-review.png" alt="Active bodies in {name}">')
    parts.append('<p>See <a href="../../BEADS6_INVENTORY.md">BEADS6_INVENTORY.md</a> for method and limitations. No new maker questions are needed for this step.</p>')
    path.write_text('\n'.join(parts))
...
def main():
    ...
    edits_path=ROOT/'photo2/beads6-review-r078.json';edits=json.loads(edits_path.read_text());image_path=ROOT/'beads6.jpg'
    ...
    old=bg.observations(rgb,old_labels,xy,old_mask,hsv,6)
    removed={r['id'] for r in edits['removed']};fragments=set(edits['fragment_ids'])
    records=[dict(id=r['observation_id'],marker_xy=r['marker_xy'],color=edits['color_overrides'].get(str(r['observation_id']),r['color']),status='ignored_fragment' if r['observation_id'] in fragments else 'reviewed_body',bead_index=None,physical_center_known=False,provenance='R059 marker reviewed from beads6 JPEG in R078') for r in old if r['observation_id'] not in removed]
    for row in edits['added_bodies']:records.append(dict(id=row['id'],marker_xy=row['xy'],color=row['color'],status='reviewed_body',bead_index=None,physical_center_known=False,provenance='R078 manual visible body',note=row['note']))
    for row in records:
    ...
    palette=np.array([[255,255,255],[220,20,20],[60,150,170],[190,190,190]],dtype=np.uint8)
    ...
    test=ROOT/'photo2/test_beads6_inventory.py'
