#!/usr/bin/env python3
"""Image-only beads2 body/color review with R069 sliver selection."""
...
COLORS=['orange','yellow','violet','lavender']
PARAMETERS=dict(minimum_chroma=12,minimum_value=35,warm_hue=[.025,.23],purple_hue=[.63,.94],
                yellow_hue_min=.105,violet_saturation_min=.45,fill_highlight_holes_below=64,
...
def color_support(rgb):
    hsv=rgb2hsv(rgb);h,s,v=np.moveaxis(hsv,-1,0)
    chroma=rgb.max(axis=2).astype(int)-rgb.min(axis=2).astype(int)
    warm=(h>PARAMETERS['warm_hue'][0])&(h<PARAMETERS['warm_hue'][1])
    purple=(h>PARAMETERS['purple_hue'][0])&(h<PARAMETERS['purple_hue'][1])
    core=(chroma>=PARAMETERS['minimum_chroma'])&(v*255>=PARAMETERS['minimum_value'])&(warm|purple)
    channels=np.zeros(h.shape,np.uint8)
    channels[core&warm]=np.where(h[core&warm]>=PARAMETERS['yellow_hue_min'],2,1)
    channels[core&purple]=np.where(s[core&purple]>PARAMETERS['violet_saturation_min'],3,4)
    mask=core.copy();components,_=ndi.label(~core);sizes=np.bincount(components.ravel())
    fill=sizes<=PARAMETERS['fill_highlight_holes_below']
    fill[np.unique(np.r_[components[0],components[-1],components[:,0],components[:,-1]])]=False
    mask|=fill[components]
    if core.any():
...
def correction_sheet(rgb,old,active,path):
    boxes=[('324: violet; excluded by area rule',(195,412,235,449)),
           ('Yellow bodies 203 / 394',(115,234,154,278)),
           ('Lavender bodies 212 / 395',(653,263,699,306)),
           ('One bottom body: 382',(418,472,455,501))]
...
def gallery(path,active,excluded,summary,boxes):
    encoded=base64.b64encode((ROOT/'beads2.jpg').read_bytes()).decode()
    parts=['<!doctype html><meta charset="utf-8"><title>beads2 active inventory</title>',
           ...
           '<h1>beads2.jpg: active body observations</h1>',
    ...
    parts.append('</svg><h2>Reviewed corrections</h2><img src="corrections.png" alt="Raw, initial and corrected body observations"><h2>Provisional active regions</h2><p>Yellow: region boundaries. Excluded regions remain unassigned; same-color borders are approximate.</p><img src="regions.png" alt="Active region boundaries">')
    for name in boxes:parts.append(f'<h2>{name}</h2><img src="{name}-review.png" alt="Active bodies in {name}">')
    parts.append('<p>See <a href="../../BEADS2_INVENTORY.md">BEADS2_INVENTORY.md</a> for method and limitations. No new maker questions are needed for this step.</p>')
    path.write_text('\n'.join(parts))
...
def main():
    ...
    edits_path=ROOT/'photo2/beads2-review-r070.json';edits=json.loads(edits_path.read_text());image_path=ROOT/'beads2.jpg'
    ...
    old=bg.observations(rgb,old_labels,xy,old_mask,hsv,2);removed={r['id'] for r in edits['removed']};fragments=set(edits['fragment_ids'])
    records=[dict(id=r['observation_id'],marker_xy=r['marker_xy'],color=edits['color_overrides'].get(str(r['observation_id']),r['color']),status='ignored_fragment' if r['observation_id'] in fragments else 'reviewed_body',bead_index=None,physical_center_known=False,provenance='R059 marker reviewed from beads2 JPEG in R070') for r in old if r['observation_id'] not in removed]
    for row in edits['added_bodies']:records.append(dict(id=row['id'],marker_xy=row['xy'],color=row['color'],status='reviewed_body',bead_index=None,physical_center_known=False,provenance='R070 manual visible body',note=row['note']))
    for row in records:
        row['marker_xy']=edits.get('marker_overrides',{}).get(str(row['id']),row['marker_xy'])
    core,mask,channels=color_support(rgb);labels=segment(rgb,channels,records)
    ...
    bg.write_json(args.review_bundle/'inventory.json',dict(summary=summary,active_observations=active,excluded_observations=excluded,removed=edits['removed'],unassigned_regions=regions,review_boxes=edits['boxes'],limitations=['Image palette and same-color boundaries provisional.','Slivers ignored, not assigned identities.','Unknown chain indices and missing bodies retained.']))
    for name,array in [('labels',labels),('active-labels',active_labels),('color-support',channels)]:np.save(args.output/f'{name}.npy',array)
    ...
    test=ROOT/'photo2/test_beads2_inventory.py'
