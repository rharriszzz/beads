#!/usr/bin/env python3
"""Image-only beads5 body/color review with R069 sliver selection."""
...
SUPPORT_CLASS={'purple':1,'gray':2,'white':2}
PARAMETERS=dict(envelope_closing_radius=10,envelope_fill_holes_below=1500,
                minimum_purple_dominance=12,minimum_saturation=.13,minimum_neutral_value=45,
                fill_color_highlight_holes_below=128,maximum_seed_snap_px=6.,
...
def color_support(rgb,envelope):
    ...
    purple=(np.minimum(rgb[:,:,0],rgb[:,:,2]).astype(int)-rgb[:,:,1].astype(int)>=PARAMETERS['minimum_purple_dominance'])&(s>=PARAMETERS['minimum_saturation'])&envelope
    channels=np.zeros(envelope.shape,np.uint8)
    channels[envelope&(v*255>=PARAMETERS['minimum_neutral_value'])]=2
    channels[purple]=1
    components,_=ndi.label(~purple);sizes=np.bincount(components.ravel())
    fill=sizes<=PARAMETERS['fill_color_highlight_holes_below']
    fill[np.unique(np.r_[components[0],components[-1],components[:,0],components[:,-1]])]=False
    channels[fill[components]&envelope]=1
    return purple,channels>0,channels
...
def correction_sheet(rgb,old,active,path):
    boxes=[('Neutral bodies and missing markers', (325,42,400,99)),
           ('Gray body versus bright glint', (78,151,128,210)),
           ('White body duplicate peaks', (674,280,724,323)),
           ('Purple and neutral edge bodies', (380,450,466,513))]
    ...
    for i,(title,box) in enumerate(boxes):
        ...
        for j,(label,rows) in enumerate([('Raw JPEG',[]),('R059 candidates',before),('R077 active',active)]):
...
def gallery(path,active,excluded,summary,boxes):
    encoded=base64.b64encode((ROOT/'beads5.jpg').read_bytes()).decode()
    parts=['<!doctype html><meta charset="utf-8"><title>beads5 active inventory</title>',
           ...
           '<h1>beads5.jpg: active body observations</h1>',
    ...
    parts.append('</svg><h2>Unresolved region warnings</h2><img src="warnings.png" alt="Raw, numbered and outlined warning regions"><h2>Corrections from JPEG review</h2><img src="corrections.png" alt="Raw, baseline and active observations"><h2>Local-area exclusions</h2><img src="area-review.png" alt="Small regions excluded by the unchanged area rule"><h2>Foreground and palette</h2><img src="envelope.png" alt="Cyan search envelope"><img src="palette.png" alt="Purple and shared neutral support"><h2>Provisional active regions</h2><p>Yellow: region boundaries. Excluded regions remain unassigned; same-color borders are approximate.</p><img src="regions.png" alt="Active region boundaries">')
    for name in boxes:parts.append(f'<h2>{name}</h2><img src="{name}-review.png" alt="Active bodies in {name}">')
    parts.append('<p>See <a href="../../BEADS5_INVENTORY.md">BEADS5_INVENTORY.md</a> for method and limitations. No new maker questions are needed for this step.</p>')
    path.write_text('\n'.join(parts))
...
def main():
    ...
    edits_path=ROOT/'photo2/beads5-review-r077.json';edits=json.loads(edits_path.read_text());image_path=ROOT/'beads5.jpg'
    ...
    old=bg.observations(rgb,old_labels,xy,old_mask,hsv,5)
    removed={r['id'] for r in edits['removed']};fragments=set(edits['fragment_ids'])
    records=[dict(id=r['observation_id'],marker_xy=r['marker_xy'],color=edits['color_overrides'].get(str(r['observation_id']),r['color']),status='ignored_fragment' if r['observation_id'] in fragments else 'reviewed_body',bead_index=None,physical_center_known=False,provenance='R059 marker reviewed from beads5 JPEG in R077') for r in old if r['observation_id'] not in removed]
    for row in edits['added_bodies']:records.append(dict(id=row['id'],marker_xy=row['xy'],color=row['color'],status='reviewed_body',bead_index=None,physical_center_known=False,provenance='R077 manual visible body',note=row['note']))
    for row in records:
    ...
    bg.write_json(args.review_bundle/'inventory.json',dict(summary=summary,active_observations=active,excluded_observations=excluded,removed=edits['removed'],unassigned_regions=regions,review_boxes=edits['boxes'],limitations=['Image-derived envelope includes uncertain neutral shadow boundaries. Gray and white share pixel support; body colors are provisional visual labels.','Neutral watershed borders, gray/white body colors and shadow separation remain provisional.','Slivers ignored, not assigned identities. Beads5 188 retains a large-area possible-merge warning; do not treat its region as a clean one-bead mask.','Unknown chain indices and missing bodies retained.']))
    ...
    palette=np.array([[255,255,255],[150,40,170],[140,140,140]],dtype=np.uint8)
    ...
    test=ROOT/'photo2/test_beads5_inventory.py'
